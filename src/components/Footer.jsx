...
const Footer = () => {
  ...
  return (
    ...
    <div className=' border-spacing-3 border-opacity-10 border rounded-3xl border-grey py-20 bg-image  ' >
      ...
      <div className='text-opacity-30 w-fit mx-auto sm:text-xl text-white sm:max-w-[30%] text-center ' >We bring your vision to life with creativity and precision. Let’s make it happen.</div>
        ...
        <ul className='flex flex-col items-center lg:flex-row justify-center text-xl sm:text-2xl ' >
          {Data.map((data, index) => (
            <Link className="flex "
              ...
              key={index}
              to={data.route} >
              <li className=' sm:mx-5 lg:my-0 my-2 hover:text-opacity-100 ease-in duration-500 cursor-pointer text-white text-opacity-50 ' >{data.Title}</li>
            </Link>
          ))}
        </ul>
      </div>
      <div className='sm:text-2xl  text-center text-white text-opacity-50 my-5 sm:my-10 ' >Copyright <span className='text-orange' >SLATY</span>. All right reserved.</div>
      </Fade>
    </div>
    </div>
  )
