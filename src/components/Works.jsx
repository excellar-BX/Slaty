...
const Works = () => {
  ...
  const Cards =[
    {title:'Share Vision', info:'Present your idea, we quickly understand your goals', icon:<BiBrain className='animate-AttentionSeeker ' /> },
    {title:'Plan Fast', info:'In one day, we craft your projects blueprint.', icon:<BiCheckCircle className='animate-AttentionSeeker ' /> },
    {title:'Launch Soon', info:'Your SaaS is developed and live in 30 days.', icon:<BiGroup className='animate-AttentionSeeker ' /> },
  ]
  ...
  return (
    <div className='sm:mb-40 mb-20 text-white ' id='h-works' >
      ...
      <div className='xl:text-6xl sm:text-4xl text-2xl font-bold sm:max-w-[60%] text-center leading-relaxed my-10 w-fit mx-auto ' >Your SaaS Brought To Life In 30 Days</div>
      </Rotate>
      <Fade direction='down' triggerOnce >
      <div className='text-opacity-30 w-fit mx-auto sm:text-xl text-center text-white ' >That drives impactful gain powerful results</div>
      </Fade>
      <div className="card grid sm:grid-cols-2 xl:grid-cols-3 items-center justify-center gap-10 my-20 ">
        ...
        {Cards.map((data, index) => (
          <div key={index} className='sm:h-[450px] h-[400px] border-spacing-3 relative overflow-hidden border-opacity-10 border border-grey rounded-xl p-10 w-[300px] mx-auto lg:w-[380px] sm:w-[400px] justify-center items-center bg-white bg-opacity-5 ' >
            ...
            <div className='text-center text-2xl sm:text-4xl font-bold  my-2 ' >{data.title}</div>
            <div className='text-white text-sm sm:text-xl text-opacity-30 text-center sm:mt-10 ' >{data.info}</div>
          </div>
        ))}
        </Fade>
      </div>
